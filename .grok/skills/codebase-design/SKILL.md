...
3. `API` remains the canonical name for Observatory's versioned public HTTP contract.
   Use **interface** for an internal module surface; do not rename the public API.
4. Treat TypeScript snippets below as language-neutral illustrations; implement in the
   repository's Python style.
5. Use real PostgreSQL tests when behavior depends on transactions, constraints, locking,
   migrations, or SQL semantics. An in-memory substitute cannot prove those claims.
6. External-provider fixtures and live adapters may share a seam, but ordinary automated
   tests must never call a provider.
7. Propose authority changes to the Project Steward; do not edit authority as a side effect.
