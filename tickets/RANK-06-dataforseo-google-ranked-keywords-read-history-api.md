...
**Status:** DRAFT — Steward code-first review complete; implementation not authorized  
...
## Steward code-first review — provisional technical boundary

This section records the Steward's read-only code-first result from repository authority,
...
API routing, and API test precedents. It is the review target for the designated Writer's
read-only pre-implementation challenge. The Writer must try to disprove these choices before
implementation. No Product question is presently known.
...
expected design computes the same outer list math locally rather than modifying
`provider_history.py`. The Writer must explicitly challenge that choice if a smaller safe
reuse exists.
...
adapter, exact ordered four-kind list above, and exact six-class Capture taxonomy above.
...
Monthly item occurrences are provisionally nested under the monthly semantic fact they cite;
...
families and a successful zero-returned-item result.
...
- keyword-info current demand, competition/CPC/bids/categories, monthly-series state,
  search-volume-trend state, signed trend member state/value testimony, and keyword-info
  source-local clock;
- properties including exact provider `core_keyword`, algorithm label, keyword difficulty,
  detected language, and another-language testimony;
- exact decimal backlink averages plus their own provider clock;
- search intent / ordered foreign intent plus its own provider clock;
- keyword-SERP URL, ordered item types, result count, and independent current/previous
  keyword-SERP clocks;
...
must be stated, and its persisted `monthly_searches_state` must be stated.
...
`item_se_type` is exact provider item testimony and Recipe v1 accepts `google`; it is not a
cross-surface engine identity.

### 12. Observation-envelope complete set and count semantics

Recompute every semantic identity from its persisted axes with the exact accepted Recipe.
...
At minimum the Writer review must enumerate and test the exact applicable domains for:

- ordinary optional state/value fields;
- rank-absolute `count`/ETV/cost inapplicability;
...
Provisional deterministic presentation:
...
- ranked results: `rank_group, rank_absolute, keyword, serp_item_type, identity` ascending;
...
## Designated Writer code-first questions

Before implementation, the explicitly designated Writer must inspect current authority,
RANK-05 source/schema/tests, sibling readers/tests, and API routing and answer these without
mutation:
...
## Provisional implementation path ceiling
...
- identity-axis tamper for each of the four kinds;
...
- matching context with missing/foreign/non-admitted Outcome returns 409;
...
## No Product question currently identified

The Steward's code-first review found no unresolved Product choice. RANK-05 already settled
the material semantic questions: four kinds, placement identity A, URL-as-content, independent
rank systems, Ranked-local keyword enrichment, monthly Data Period identity, whole-unit
conflict behavior, Product Option 1 prose retention, zero-item semantics, and the four-pillar
time model.

If the designated Writer identifies a real Product choice, stop before implementation and
return it through the question-resolution gate. Otherwise the expected next step is Steward
reconciliation of the Writer's read-only review into a final accepted RANK-06 ticket, followed
only then by a separate [CHAZ] implementation authorization from the exact clean accepted
ticket HEAD.
