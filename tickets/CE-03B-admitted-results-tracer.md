...
- Acceptance evidence:
  ...
  - D8 structural gate (closure-held issuance registry; no module token):
    ...
    `test_public_runner_rejects_lying_evidencestore_subclass_before_transport`
...
- Unproven limits:
  ...
  - D8 gate: ordinary module/API construction, including a duck-typed store
    or an `EvidenceStore` subclass passed to `capture_admitted_results`. Not a
    claim against monkeypatching methods on a genuine concrete instance,
    function replacement, or closure-cell introspection.
- Review findings remaining:
  - Public surface: `capture_admitted_results(store, inputs)`,
    ...
    request bytes at issuance. Issuance requires
    `type(store) is EvidenceStore` (the concrete class from
    `create_store`/`open_store`). Subclass substitution is rejected. This is
    not a claim against `object.__new__` plus closure-cell introspection or
    monkeypatching methods on a genuine concrete instance.
